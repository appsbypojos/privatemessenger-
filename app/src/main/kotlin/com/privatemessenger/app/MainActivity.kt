...
class MainActivity : AppCompatActivity() {
    ...
    private fun showAuth() {
        ...
        val root = baseRoot().apply {
            ...
            setPadding(28.dp(), 24.dp(), 28.dp(), 20.dp())
        }
        ...
        root.addView(register, buttonLp())
        root.addView(TextView(this).apply {
            text = "Ник: 3–24 символа • a-z, 0-9, _"
            textSize = 13f
            gravity = Gravity.CENTER
    ...
    private fun authenticate(username: String, password: String, register: Boolean) {
        ...
        scope.launch {
            try {
                if (register) {
                    withContext(Dispatchers.IO) { SupabaseClient.register(nick, password) }
                } else {
                    withContext(Dispatchers.IO) { SupabaseClient.signIn(nick, password) }
                }
                showPeople()
            } catch (e: Exception) {
    ...
    private fun showPeople() {
        ...
        header.addView(logout, LinearLayout.LayoutParams(-2, 52.dp()))
        root.addView(header)
        ...
        fun render(arr: org.json.JSONArray) {
            ...
            if (arr.length() == 0) {
                results.addView(TextView(this).apply {
                    text = if (search.text.isNullOrBlank()) "Пользователи не найдены" else "Ничего не найдено"
                    textSize = 16f
                    setTextColor(Color.GRAY)
        ...
        search.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                searchJob = scope.launch {
                    delay(220)
                    try {
                        val arr = withContext(Dispatchers.IO) { SupabaseClient.searchProfiles(s?.toString().orEmpty()) }
                        render(arr)
                    } catch (e: Exception) {
        ...
        scope.launch {
            try {
                render(withContext(Dispatchers.IO) { SupabaseClient.searchProfiles("") })
            } catch (e: Exception) {
                toast(e.message ?: "Не удалось загрузить пользователей")
            }
        }
    }
    ...
    private fun openChat(otherUserId: String, name: String) {
        ...
        header.addView(back, LinearLayout.LayoutParams(56.dp(), 52.dp()))
        header.addView(title, LinearLayout.LayoutParams(0, 52.dp(), 1f))
        root.addView(header)
        ...
        row.addView(send, LinearLayout.LayoutParams(58.dp(), 54.dp()))
        root.addView(row)
        ...
        scope.launch {
            try {
                ...
                pollingJob = launch {
                    while (isActive) {
                        delay(1500)
                        try { refresh(cid) } catch (_: Exception) {}
                    }
                }
            } catch (e: Exception) {
