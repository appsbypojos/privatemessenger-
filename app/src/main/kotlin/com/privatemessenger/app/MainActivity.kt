...
class MainActivity : AppCompatActivity() {
    ...
    private fun showLogin() {
        ...
        root.addView(title, lp()); root.addView(subtitle, lp()); root.addView(email, lp()); root.addView(password, lp()); root.addView(login, lp()); root.addView(register, lp())
        setContentView(root)
    ...
    private fun authenticate(email: String, password: String, signup: Boolean) {
        ...
        scope.launch {
            try {
                ...
                if (signup && SupabaseClient.accessToken == null) toast("Аккаунт создан. Подтвердите email и затем войдите.") else showUsers()
            } catch (e: Exception) { toast(e.message ?: "Ошибка авторизации") }
        }
    }

    private fun showUsers() {
        pollingJob?.cancel(); conversationId = null
        ...
        root.addView(header); root.addView(scroll, LinearLayout.LayoutParams(-1, 0, 1f)); setContentView(root)
        ...
        scope.launch {
            try {
                ...
                for (i in 0 until arr.length()) {
                    val p = arr.getJSONObject(i); val id = p.getString("id")
    ...
    private fun openChat(otherUserId: String, name: String) {
        ...
        root.addView(header); root.addView(messageScroll, LinearLayout.LayoutParams(-1, 0, 1f)); root.addView(row); setContentView(root)
        back.setOnClickListener { pollingJob?.cancel(); showUsers() }
        send.setOnClickListener {
            val text = input.text.toString().trim(); if (text.isBlank()) return@setOnClickListener
            ...
            scope.launch {
                try {
                    ...
                    withContext(Dispatchers.IO) { SupabaseClient.send(cid, text) }; input.text.clear(); refresh(cid)
                } catch (e: Exception) { toast(e.message ?: "Ошибка отправки") } finally { send.isEnabled = true }
            }
        }
        scope.launch {
            try {
                ...
                pollingJob = launch { while (isActive) { delay(2000); try { refresh(conversationId!!) } catch (_: Exception) {} } }
    ...
    private suspend fun refresh(cid: String) {
        ...
        for (i in 0 until arr.length()) {
            ...
            val bubble = TextView(this).apply { this.text = text; textSize = 17f; setTextColor(Color.BLACK); setPadding(18, 12, 18, 12); gravity = if (mine) Gravity.END else Gravity.START }
