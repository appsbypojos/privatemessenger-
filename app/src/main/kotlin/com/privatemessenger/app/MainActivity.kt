...
class MainActivity : AppCompatActivity() {
    ...
    override fun onDestroy() { pollingJob?.cancel(); searchJob?.cancel(); scope.cancel(); super.onDestroy() }
    ...
    private fun applyInsets(root: View) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(root) { view, insets ->
            ...
            view.setPadding(baseLeft, baseTop + bars.top, baseRight, baseBottom + maxOf(bars.bottom, ime.bottom)); insets
        }
    ...
    private fun authenticate(username: String, password: String, register: Boolean) {
        ...
        scope.launch { try {
            withContext(Dispatchers.IO) { if (register) SupabaseClient.register(nick, password) else SupabaseClient.signIn(nick, password) }
            showPeople()
    ...
    private fun showPeople() {
        ...
        fun render(arr: org.json.JSONArray) {
            results.removeAllViews(); if (arr.length() == 0) { results.addView(TextView(this).apply { text = if (search.text.isNullOrBlank()) "Пользователи не найдены" else "Ничего не найдено"; textSize = 16f; setTextColor(Color.GRAY); setPadding(8.dp(), 24.dp(), 8.dp(), 24.dp()) }); return }
            for (i in 0 until arr.length()) { val p = arr.getJSONObject(i); val id = p.getString("id"); val nick = p.optString("username"); val button = Button(this).apply { text = "@$nick"; textSize = 17f; isAllCaps = false; gravity = Gravity.START or Gravity.CENTER_VERTICAL }; results.addView(button, LinearLayout.LayoutParams(-1, 58.dp()).apply { bottomMargin = 6.dp() }); button.setOnClickListener { openChat(id, nick) } }
        }
        search.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(s: Editable?) = Unit; override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) = Unit
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) { searchJob?.cancel(); searchJob = scope.launch { delay(220); try { render(withContext(Dispatchers.IO) { SupabaseClient.searchProfiles(s?.toString().orEmpty()) }) } catch (e: Exception) { toast(e.message ?: "Ошибка поиска") } } }
        })
    ...
    private fun openChat(otherUserId: String, name: String) {
        pollingJob?.cancel(); conversationId = null; lastMessageCount = -1
        val root = baseRoot(); val header = LinearLayout(this).apply { gravity = Gravity.CENTER_VERTICAL; setPadding(8.dp(), 4.dp(), 8.dp(), 4.dp()) }
        val back = Button(this).apply { text = "‹"; textSize = 26f; isAllCaps = false }; val chatTitle = TextView(this).apply { text = "@$name"; textSize = 21f; typeface = Typeface.DEFAULT_BOLD; setPadding(10.dp(), 0, 0, 0) }
        ...
        messageList = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(12.dp(), 12.dp(), 12.dp(), 12.dp()) }; messageScroll = ScrollView(this).apply { addView(messageList) }; root.addView(messageScroll, LinearLayout.LayoutParams(-1, 0, 1f))
        val row = LinearLayout(this).apply { gravity = Gravity.BOTTOM; setPadding(8.dp(), 6.dp(), 8.dp(), 6.dp()) }; val input = EditText(this).apply { hint = "Сообщение"; inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_FLAG_MULTI_LINE; maxLines = 4 }; val send = Button(this).apply { text = "➤"; isAllCaps = false }
        row.addView(input, LinearLayout.LayoutParams(0, -2, 1f)); row.addView(send, LinearLayout.LayoutParams(58.dp(), 54.dp())); root.addView(row); setContentView(root); applyInsets(root); back.setOnClickListener { pollingJob?.cancel(); showPeople() }
        send.setOnClickListener { val text = input.text.toString().trim(); if (text.isBlank()) return@setOnClickListener; send.isEnabled = false; scope.launch { try { val cid = conversationId ?: withContext(Dispatchers.IO) { SupabaseClient.directConversation(otherUserId) }.also { conversationId = it }; withContext(Dispatchers.IO) { SupabaseClient.send(cid, text) }; input.text.clear(); refresh(cid) } catch (e: Exception) { toast(e.message ?: "Ошибка отправки") } finally { send.isEnabled = true } } }
    ...
    private suspend fun refresh(cid: String) {
        val arr = withContext(Dispatchers.IO) { SupabaseClient.messages(cid) }; if (arr.length() == lastMessageCount) return; lastMessageCount = arr.length(); messageList.removeAllViews()
        for (i in 0 until arr.length()) { val o = arr.getJSONObject(i); val mine = o.optString("sender_id") == SupabaseClient.userId; val bubble = TextView(this).apply { text = o.optString("body"); textSize = 16f; setTextColor(Color.BLACK); setPadding(16.dp(), 10.dp(), 16.dp(), 10.dp()); gravity = if (mine) Gravity.END else Gravity.START }; messageList.addView(bubble, LinearLayout.LayoutParams(-1, -2).apply { topMargin = 3.dp(); bottomMargin = 3.dp() }) }
