...
class MainActivity : AppCompatActivity() {
    ...
    private fun openChatInternal(cid:String,name:String){
        chatPoll?.cancel();lastCount=-1;val r=root();val header=LinearLayout(this).apply{gravity=Gravity.CENTER_VERTICAL;background=surface;setPadding(dp(5),dp(2),dp(12),dp(2))};val back=Button(this).apply{text="‹";textSize=30f;isAllCaps=false;setTextColor(accent);background=null};header.addView(back,lp(dp(52),dp(56)));val av=TextView(this).apply{text=name.take(1).uppercase();textSize=17f;gravity=Gravity.CENTER;setTextColor(Color.WHITE);background=gradient()};header.addView(av,lp(dp(44),dp(44)).apply{rightMargin=dp(10)});val h=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};h.addView(tv("@$name",18f,ink,true));h.addView(tv("Nexo",12f,muted));header.addView(h,lp(0,dp(56)).apply{weight=1f});r.addView(header)
