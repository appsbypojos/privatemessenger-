...
class NexoActivity : AppCompatActivity() {
    ...
    private fun icon(id: Int, size: Int = 28, alpha: Float = 1f) = ImageView(this).apply {
        setImageResource(id); this.alpha=alpha; scaleType=ImageView.ScaleType.CENTER_INSIDE
    }
    ...
    private fun nav():View{
        ...
        for(i in ids.indices){val cell=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;gravity=Gravity.CENTER;isClickable=true;setPadding(0,dp(3),0,dp(2))};val iv=icon(ids[i],25,if(i==tab)1f else .45f);cell.addView(iv,lp(dp(34),dp(28)));cell.addView(text(names[i],10f,if(i==tab) blue else muted,i==tab).apply{gravity=Gravity.CENTER});bar.addView(cell,lp(0,dp(56)).apply{weight=1f});cell.setOnClickListener{home(i)}}
        return bar
    }
    ...
    private fun openChat(cid:String,name:String){pollJob?.cancel();currentChat=cid
        val r=root();val head=LinearLayout(this).apply{gravity=Gravity.CENTER_VERTICAL;setBackgroundColor(surface);setPadding(dp(5),dp(2),dp(10),dp(2))};val back=iconButton(R.drawable.ic_back,50);head.addView(back,lp(dp(50),dp(56)));val av=text(name.take(1).uppercase().ifBlank{"N"},17f,Color.WHITE,true).apply{gravity=Gravity.CENTER;background=grad()};head.addView(av,lp(dp(44),dp(44)).apply{rightMargin=dp(10)});val titles=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};titles.addView(text("@$name",18f,ink,true));titles.addView(text("Nexo",12f,muted));head.addView(titles,lp(0,dp(56)).apply{weight=1f});val more=iconButton(R.drawable.ic_more,44);head.addView(more,lp(dp(44),dp(50)));r.addView(head)
        ...
        val composer=LinearLayout(this).apply{gravity=Gravity.BOTTOM;setBackgroundColor(surface);setPadding(dp(7),dp(6),dp(7),dp(6))};val attach=iconButton(R.drawable.ic_attach,44);val input=EditText(this).apply{hint="Сообщение";inputType=InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_FLAG_MULTI_LINE;maxLines=4;background=round(bg,22);setPadding(dp(14),dp(5),dp(10),dp(5))};val mic=iconButton(R.drawable.ic_mic,44);val send=iconButton(R.drawable.ic_send,48,grad());composer.addView(attach,lp(dp(44),dp(50)));composer.addView(input,lp(0,-2).apply{weight=1f;leftMargin=dp(3);rightMargin=dp(3)});composer.addView(mic,lp(dp(44),dp(50)));composer.addView(send,lp(dp(48),dp(50)));r.addView(composer);setContentView(r);insets(r)
